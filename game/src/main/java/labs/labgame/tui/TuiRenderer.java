...
public class TuiRenderer {
    ...
    private static final float CELLS_PER_WORLD_X = 4.0f;
    private static final float CELLS_PER_WORLD_Y = 2.0f;
    ...
    public Viewport createViewport(TerminalSize size) {
        ...
        float worldWidth = size.getColumns() / CELLS_PER_WORLD_X;
        float worldHeight = gameRows / CELLS_PER_WORLD_Y;
    ...
    public void render(Screen screen) {
        ...
        if (size.getRows() <= HUD_ROWS + 4 || size.getColumns() < 32) {
            TextGraphics graphics = screen.newTextGraphics();
            graphics.setForegroundColor(TextColor.ANSI.WHITE);
            graphics.putString(0, 0, "Terminal too small for the game view");
            return;
        }
        ...
        for (Effect effect : model.getEffects()) {
            if (effect instanceof ExplosionEffect explosion) {
                ...
                char symbol = effect.getProgress() < 0.45f ? 'o' : '.';
                TextColor color = effect.getProgress() < 0.45f ? TextColor.ANSI.RED : TextColor.ANSI.YELLOW;
                drawCircle(screen, effect.getPosition().x, effect.getPosition().y, radius, symbol,
                        color, viewport);
            }
        }
    ...
    public record Viewport(float left, float bottom, float top, float right, int gameRows, int columns) {
        ...
        public Vector2 cellCenterToWorld(int column, int row) {
            float worldX = left + (column + 0.5f) / CELLS_PER_WORLD_X;
            float worldY = bottom + (gameRows - (row - HUD_ROWS) - 0.5f) / CELLS_PER_WORLD_Y;
    ...
    private void drawHud(TextGraphics graphics, TerminalSize size, Player player, int enemies) {
        ...
        String hudTop = String.format("Time %5.1f Enemies %d Best %d Score %d HP [%s] %3.0f  ",
                model.getTime(), enemies, player.maxScore, player.score, healthBar, player.getHealth());
    ...
    private void drawBackground(Screen screen, Viewport viewport) {
        ...
        int leftCell = (int) Math.floor(viewport.left() * CELLS_PER_WORLD_X);

        for (int row = HUD_ROWS; row < gameRowEnd; row++) {
            float worldY = viewport.bottom() + (viewport.gameRows() - (row - HUD_ROWS) - 0.5f) / CELLS_PER_WORLD_Y;
            float worldX = viewport.left() + 0.5f / CELLS_PER_WORLD_X;
            ...
            for (int column = 0; column < columns; column++) {
                ...
                worldX += 1.0f / CELLS_PER_WORLD_X;
            }
        }
    }

    private void drawGround(Screen screen, Viewport viewport) {
        int row = worldToRow(0.0f, viewport.top());
        if (row < HUD_ROWS || row >= HUD_ROWS + viewport.gameRows()) {
            return;
        }

        for (int column = 0; column < viewport.columns(); column++) {
            setCell(screen, column, row, '=', TextColor.ANSI.YELLOW);
        }

        int tickSpacing = Math.max(2, Math.round(CELLS_PER_WORLD_X * 5.0f));
        int startColumn = tickSpacing - Math.floorMod((int) Math.floor(viewport.left() * CELLS_PER_WORLD_X), tickSpacing);
        for (int column = startColumn; column < viewport.columns(); column += tickSpacing) {
            setCell(screen, column, row, '+', TextColor.ANSI.GREEN);
        }
    }

    private void drawPlayer(Screen screen, Player player, Viewport viewport) {
        drawRect(screen, player.getX(), player.getY(), player.getWidth(), player.getHeight() + 0.2f, '%', TextColor.ANSI.GREEN, viewport);
    }

    private void drawEnemy(Screen screen, Enemy enemy, Viewport viewport) {
        drawRect(screen, enemy.getX(), enemy.getY(), enemy.getWidth(), enemy.getHeight(), 'H', TextColor.ANSI.RED, viewport);
    }

    private void drawBlock(Screen screen, Block block, Viewport viewport) {
        drawRect(screen, block.getX(), block.getY(), block.getSize(), block.getSize(), '#', TextColor.ANSI.YELLOW, viewport);
    }

    private void drawBullet(Screen screen, Bullet bullet, Viewport viewport) {
        drawCircle(screen, bullet.getX(), bullet.getY(), Bullet.RADIUS, 'o', TextColor.ANSI.WHITE, viewport);
    }
    ...
    private void drawRect(Screen screen, float centerX, float centerY, float width, float height, char symbol,
                          TextColor color, Viewport viewport) {
    ...
    private void drawCircle(Screen screen, float centerX, float centerY, float radius, char symbol, TextColor color,
                            Viewport viewport) {
    ...
    private int worldToRow(float worldY, float viewportTop) {
        int rowInGame = (int) Math.floor((viewportTop - worldY) * CELLS_PER_WORLD_Y);
        return HUD_ROWS + rowInGame;
    }

    private int clampColumn(float worldX, Viewport viewport) {
        int column = (int) Math.floor((worldX - viewport.left()) * CELLS_PER_WORLD_X);
        return Math.max(0, column);
    }

    private int clampColumnEnd(float worldX, Viewport viewport) {
        int column = (int) Math.ceil((worldX - viewport.left()) * CELLS_PER_WORLD_X);
        return Math.min(viewport.columns() - 1, column);
    }

    private int clampTopRow(float worldY, Viewport viewport) {
        return Math.max(HUD_ROWS, worldToRow(worldY, viewport.top()));
    }

    private int clampBottomRow(float worldY, Viewport viewport) {
        return Math.min(HUD_ROWS + viewport.gameRows() - 1, worldToRow(worldY, viewport.top()));
    }

    private float columnToWorldX(int column, Viewport viewport) {
        return viewport.left() + (column + 0.5f) / CELLS_PER_WORLD_X;
    }

    private float rowToWorldY(int row, Viewport viewport) {
        return viewport.bottom() + (viewport.gameRows() - (row - HUD_ROWS) - 0.5f) / CELLS_PER_WORLD_Y;
    }
